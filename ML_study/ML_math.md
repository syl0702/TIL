...
- 선형연립방정식의 해가 영벡터밖에 없으면 선형독립.
- 해가 무한히 많은 경우 그 중에 영벡터가 아닌 해가 반드시 존재하므로 선형 종속이다.

### Tip
- 피처 갯수보다 행 갯수 많아야 함.
- N >= M
- 선형 종속이 좋은게 아니다!
- 관련 없는 데이터(평균 등)를 추가하면 다중공선성 문제가 생길 수 있다.
- 이러한 집계된 값은 빼는 것이 좋다.
...
## 풀랭크
- 랭크가 행의 갯수와 열의 갯수 중 작은 값과 같으면 **풀랭크(Full Rank)**
- 선형독립인 벡터들을 행 또는 열로 가지는 행렬을 만들면 항상 풀랭크!
- 정방행렬이 플랭크다 <-> 역행렬이 존재한다

## 로우-랭크 행렬

## 벡터공간과 기저벡터
- 벡터 N개가 서로 선형독립이면 이 벡터들을 선형조합하여 만들어지는 모든 벡터의 집합을 벡터공간(vectorSpace) V라고 하고 이 벡터공간의 차원은 N.
- 이 벡터들을 벡터공간의 **기저벡터(basis vector)**라고 함.

## 정규직교인 기저벡터로 이루어진 벡터공간
- 직교벡터는 기저벡터로 이루어진 벡터공간의 모든 벡터에 대해 직교.
- 벡터 x의 투영벡터는 기저벡터로 이루어진 벡터공간의 모든 벡터 중에서 가장 벡터 x와 가까운 벡터.
