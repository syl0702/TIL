...
## 크레인 인형뽑기 게임
- 다시 풀 것.
- `.pop()` 은 하나를 빼기 때문에 그 이후 배열이 달라짐.
- 즉 `out of range` 에러가 나올 수 있다는 뜻이므로 코드 짤 때 주의 할 것.

## 키패드 누르기
- 다시 풀 것.
- Dictionary 형태가 가장 직관적.
- 어떤 케이스들이 나오는지 정리를 하고 코드를 작성할 것. 무작정 모든 케이스라고 고려하면 안됨.

## 신규 아이디 추천
- 다시 풀 것.
- 굳이 새 리스트를 만들기 보다는 있는 것을 최대한 활용하여 코드 짜기.